  ...
  describe("Integration with rustify types", () => {
    ...
    test("Option/Result patterns work on `unknown` input via P.any narrowing", () => {
      ...
      const classify = (v: unknown): string =>
        match(v)
          .with(
            P.when((x): x is Option<number> =>
              Option.isOption(x) && x.isSome()
            ),
            (s) => `Some(${s.unwrap()})`,
          )
          .with(
            P.when((x): x is Option<number> =>
              Option.isOption(x) && x.isNone()
            ),
            () => "None",
          )
          .with(
            P.when((x): x is Result<number, string> => Result.isResult(x)),
            (r) => r.isOk() ? `Ok(${r.unwrap()})` : `Err(${r.unwrapErr()})`,
          )
          .with(P.any, () => "other")
          .exhaustive();
