...
3. Publishing to JSR happens via .github/workflows/publish.yml, triggered by the `release: published` event — i.e. only when the draft release is actually published (marked live), NOT on tag push. The workflow runs `deno publish` from GitHub Actions (OIDC), which generates the SLSA provenance statement on jsr.io. Pre-release GitHub releases are skipped (guard `if: github.event.release.prerelease == false`). NOTE: currently hardcoded to `deno publish --dry-run` (TODO in file) until the real publish path is validated — flip the TODO before expecting an actual publish. Verify the Publish run succeeds after publishing the release; do NOT publish locally (local `deno publish` with browser auth produces NO provenance). One-time prerequisite: the package must be linked to the GitHub repo in JSR package settings (jsr.io → package → Settings → Link repository).
