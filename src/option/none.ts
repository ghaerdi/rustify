...
export class NoneStrategy<T> implements BaseOptionStrategy<T> {
  ...
  isSomeAnd(_fn: (value: T) => boolean): boolean { return false; }
  isNone(): boolean { return true; }
  expect(message: string): T { throw new Error(message); }
  unwrap(): T { throw new Error('Tried to unwrap a None value'); }
  unwrapOr(defaultValue: T): T { return defaultValue; }
  unwrapOrElse(fn: () => T): T { return fn(); }
  map<U>(_fn: (value: T) => U): NoneStrategy<U> { return new NoneStrategy<U>(); }
  mapOr<U>(defaultValue: U, _fn: (value: T) => U): U { return defaultValue; }
  mapOrElse<U>(defaultFn: () => U, _fn: (value: T) => U): U { return defaultFn(); }
  inspect(_fn: (value: T) => void): this { return this; }
  and<U>(_res: BaseOptionStrategy<U>): BaseOptionStrategy<U> { return new NoneStrategy<U>(); }
  andThen<U>(_fn: (value: T) => BaseOptionStrategy<U>): BaseOptionStrategy<U> { return new NoneStrategy<U>(); }
  or(res: BaseOptionStrategy<T>): BaseOptionStrategy<T> { return res; }
  orElse(fn: () => BaseOptionStrategy<T>): BaseOptionStrategy<T> { return fn(); }
  xor(optb: BaseOptionStrategy<T>): BaseOptionStrategy<T> { return optb; }
  cloned(): BaseOptionStrategy<T> { return this; }
  zip<U>(_other: BaseOptionStrategy<U>): BaseOptionStrategy<[T, U]> { return new NoneStrategy<[T, U]>(); }
  zipWith<U, R>(_other: BaseOptionStrategy<U>, _fn: (s: T, o: U) => R): BaseOptionStrategy<R> { return new NoneStrategy<R>(); }
